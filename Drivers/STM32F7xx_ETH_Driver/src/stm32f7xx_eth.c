...
/** @addtogroup STM32F7xx_ETH_Driver
...
/**
  * @brief  Clear Tx Buffer unavailable flag and resume transmission
  * @param  None.
  * @retval error status
  */
uint32_t ETH_ClearTransmitBufferUnavailable(void)
{
...
/**
  ...
  * @retval None
  */
void ETH_DMARxDescChainInit(ETH_DMADescBufTypeDef *DMARxDescTab, ETH_RxBufferTypeDef *RxBuff, uint32_t RxBuffCount)
{
  uint32_t i = 0;
  ETH_DMADescBufTypeDef *DMARxDescBuf;
  
  /* Set the DMARxDescToGet pointer with the first one of the DMARxDescTab list */
  DMARxDescToGet = &DMARxDescTab->DMADesc; 
  /* Fill each DMARxDesc descriptor with the right values */
  for(i=0; i < RxBuffCount; i++)
  {
    /* Get the pointer on the ith member of the Rx Desc list */
    DMARxDescBuf = DMARxDescTab+i;
    /* Set Own bit of the Rx descriptor Status */
    DMARxDescBuf->DMADesc.Status = ETH_DMARxDesc_OWN;

    /* Set Buffer1 size and Second Address Chained bit */
    DMARxDescBuf->DMADesc.ControlBufferSize = ETH_DMARxDesc_RCH | (uint32_t)ETH_RX_BUF_SIZE;  
    /* Set Buffer1 address pointer */
    DMARxDescBuf->DMADesc.Buffer1Addr = (uint32_t)(&RxBuff[i].Buffer[0]);

    /* Initialize the next descriptor with the Next Descriptor Polling Enable */
    if(i < (RxBuffCount-1))
    {
      /* Set next descriptor address register with next descriptor base address */
      DMARxDescBuf->DMADesc.Buffer2NextDescAddr = (uint32_t)&((DMARxDescTab+i+1)->DMADesc); 
    }
    else
    {
      /* For last descriptor, set next descriptor address register equal to the first descriptor base address */ 
      DMARxDescBuf->DMADesc.Buffer2NextDescAddr = (uint32_t)(&DMARxDescTab->DMADesc); 
    }
  }

  /* Set Receive Descriptor List Address Register */
  ETH->DMARDLAR = (uint32_t)&DMARxDescTab->DMADesc;
...
/**
  ...
  * @retval None
  */
void ETH_DMATxDescChainInit(ETH_DMADescBufTypeDef *DMATxDescTab, ETH_TxBufferTypeDef* TxBuff, uint32_t TxBuffCount)
{
  uint32_t i = 0;
  ETH_DMADescBufTypeDef *DMATxDescBuf;
  
  /* Set the DMATxDescToSet pointer with the first one of the DMATxDescTab list */
  DMATxDescToSet = &DMATxDescTab->DMADesc;
  /* Fill each DMATxDesc descriptor with the right values */   
  for(i=0; i < TxBuffCount; i++)
  {
    /* Get the pointer on the ith member of the Tx Desc list */
    DMATxDescBuf = DMATxDescTab + i;
    /* Set Second Address Chained bit */
    DMATxDescBuf->DMADesc.Status = ETH_DMATxDesc_TCH;  

    /* Set Buffer1 address pointer */
    DMATxDescBuf->DMADesc.Buffer1Addr = (uint32_t)(&TxBuff[i].Buffer[0]);
    
    /* Initialize the next descriptor with the Next Descriptor Polling Enable */
    if(i < (TxBuffCount-1))
    {
      /* Set next descriptor address register with next descriptor base address */
      DMATxDescBuf->DMADesc.Buffer2NextDescAddr = (uint32_t)&((DMATxDescTab+i+1)->DMADesc);
    }
    else
    {
      /* For last descriptor, set next descriptor address register equal to the first descriptor base address */ 
      DMATxDescBuf->DMADesc.Buffer2NextDescAddr = (uint32_t)&DMATxDescTab->DMADesc;  
    }
  }

  /* Set Transmit Desciptor List Address Register */
  ETH->DMATDLAR = (uint32_t)&DMATxDescTab->DMADesc;
}
