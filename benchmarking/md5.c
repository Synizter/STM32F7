 ...
 */
void ul_MD5Init(UL_MD5Context *ctx)
{
 ...
 * of bytes.
 */
void ul_MD5Update(UL_MD5Context *ctx, unsigned char const *buf, unsigned len)
{
    uint32_t t;
 ...
 */
void ul_MD5Final(unsigned char digest[UL_MD5LENGTH], UL_MD5Context *ctx)
{
    unsigned count;
    unsigned char *p;
...
{
  ul_MD5Init(&md5_enc);
  ul_MD5Transform(md5_enc.buf, (const uint32_t*)str);
}