...
#define ETH_DMA_BUFFER_MASK     (32 - 1)
#define ETH_ADDR_ALIGN_UP(n)    (((n) + ETH_DMA_BUFFER_MASK) & ~ETH_DMA_BUFFER_MASK)

#define ETH_DMADESC_PADSIZE     ETH_ADDR_ALIGN_UP(sizeof(ETH_DMADESCTypeDef))

#define ETH_DMADESC_NEXT(desc)  ((ETH_DMADESCTypeDef*)desc->Buffer2NextDescAddr)
...
typedef union {
  uint8_t pad[ETH_DMADESC_PADSIZE];
  ETH_DMADESCTypeDef DMADesc;
} ETH_DMADescBufTypeDef;
...
#define  ETH_TXBUFFER_PADSIZE           ETH_ADDR_ALIGN_UP(ETH_TX_BUF_SIZE)
#define  ETH_RXBUFFER_PADSIZE           ETH_ADDR_ALIGN_UP(ETH_RX_BUF_SIZE)

typedef union {
  uint8_t pad[ETH_TXBUFFER_PADSIZE];
  uint8_t Buffer[ETH_TX_BUF_SIZE];
} ETH_TxBufferTypeDef;

typedef union {
  uint8_t pad[ETH_RXBUFFER_PADSIZE];
